# HOLA, MI NOMBRE ES **BIVIAN STEPHANNY** 👋✨

<p><img align="right" src="https://github.com/Adam-pw/Adam-pw/blob/main/animation_500_kxa883sd.gif" width="260px"/></p>

Soy desarrolladora de software con un gusto especial por las interfaces bonitas, el diseño limpio y las aplicaciones que realmente funcionan. Tengo un pie en el mundo móvil con Flutter, otro en el backend con Python, Java y PHP, y las manos metidas donde haya código que mejorar.

Me gusta construir ideas desde cero, entender cómo se conectan las cosas y aprender tecnologías nuevas que enreden la mente de forma deliciosa.

---

## 🌿 Sobre mí

✨ Me encanta crear aplicaciones con buen diseño, buena lógica y buen corazón.  
📱 Desarrollo móvil con Flutter y apps funcionales para usuarios reales.  
🧩 Backend con Django, Spring Boot y PHP cuando toca armar cosas más grandes.  
🗂️ Bases de datos SQL y NoSQL para guardar mundos enteros.  
🎨 Aesthetic lover: colores suaves, UI limpia, organización que da paz.  
🌱 Siempre aprendiendo algo nuevo (y rompiendo cosas en el proceso, para luego arreglarlas mejor).

---

## 🚀 Tecnologías que uso

<p align="left">

<img height="40" src="https://cdn.worldvectorlogo.com/logos/flutter-logo.svg" alt="Flutter"/>&emsp;
<img height="40" src="https://kinsta.com/es/wp-content/uploads/sites/8/2018/05/PHP_Feature-Image-1024x536.jpg" alt="PHP"/>&emsp;
<img height="30" src="https://img.shields.io/badge/JavaScript-%23F7DF1E.svg?logo=javascript&logoColor=black"/>&emsp;
<img height="30" src="https://img.shields.io/badge/Python-%2314354C.svg?logo=python&logoColor=white"/>&emsp;
<img height="30" src="https://img.shields.io/badge/-Git-red?logo=git&logoColor=white"/>&emsp;
<img height="30" src="https://img.shields.io/badge/-Django-green?logo=django&logoColor=white"/>&emsp;
<img height="30" src="https://img.shields.io/badge/-MongoDB-green?logo=mongodb&logoColor=white"/>&emsp;
<img height="30" src="https://img.shields.io/badge/-GitHub-black?logo=github&logoColor=white"/>&emsp;
<img height="34" src="https://www.mysql.com/common/logos/logo-mysql-170x115.png" alt="MySQL"/>&emsp;
<img height="30" src="https://cdn.worldvectorlogo.com/logos/postgresql.svg" alt="PostgreSQL"/>

</p>

---

## 🌐 Conecta conmigo

<p align="left">
  <a href="https://www.linkedin.com/in/bivian-cruz-109739211" target="_blank">
    <img height="40" src="https://cdn-icons-png.flaticon.com/512/174/174857.png"/>
  </a>
  &emsp;
  <a href="https://www.instagram.com/bivistephy/" target="_blank">
    <img height="40" src="https://cdn-icons-png.flaticon.com/512/1409/1409946.png"/>
  </a>
</p>

---

## 📊 Mis estadísticas  
*(Generadas con mi usuario **Stephanny9313**)*

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=Stephanny9313&show_icons=true&count_private=true&theme=darcula&hide_border=true&bg_color=00000000"/>
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Stephanny9313&layout=compact&hide_border=true&theme=darcula&bg_color=00000000&langs_count=6"/>
  <img height="160" src="https://github-readme-streak-stats.herokuapp.com?user=Stephanny9313&theme=darcula&hide_border=true&background=FFFFFF00"/>
</p>

---


